...
import fileNames from "../scripts/java_file_setup.mjs";
...
class gptTool {
    ...
    async readFileContents(fileName) {
        try {
            ...
            const file = await readFile(
                new URL(`../resources/code_files/Original_Code/${fileName}`, import.meta.url),
                "utf-8"
            );
            return file
        } catch (error) {
    ...
    async writeNewFile(fileContents, fileName) {
        ...
        // write the file
        const fileNPath = '../resources/code_files/Refactor_Code/' + newFileName;
        try {
